...
import { _0x8_MATH } from './MathOpcodes'
...
export function _0xB_JUMP_V0(cpu: Chip8, opcode: number) {
  cpu.programCounter = NNN(opcode) + cpu.v[0];
}
...
export function _0xE_SKIP_IF_KEYPRESS(cpu: Chip8, opcode: number) {
  const kk = KK(opcode);
  const keyPressed = cpu.keys.indexOf(cpu.v[X(opcode)])
  switch (kk) {
    case 0xA1:
      // if input is pressed
...
export function _0xF_MISC(cpu: Chip8, opcode: number) {
  switch (KK(opcode)) {
    case 0x07:
      ...
      break;
    case 0x0A:
      _0xFX0A_WAIT_FOR_KEY(cpu, opcode);
      break;
    case 0x15:
...
export function _0xFX1E_ADD_I_VX(cpu: Chip8, opcode: number) {
  cpu.i += cpu.v[X(opcode)];
}

export function _0xFX0A_WAIT_FOR_KEY(cpu: Chip8, opcode: number) {
  const x = X(opcode);
  if (cpu.keys.length !== 0) {
    cpu.v[x] = cpu.keys[0];
  }
  else {
    cpu.programCounter -= 2;
...
export function _0xFX33_STORE_V_IN_MEMORY(cpu: Chip8, opcode: number) {
  const vx = cpu.v[X(opcode)]
  cpu.ram[cpu.i] = ~~(cpu.v[X(opcode)] / 100);
  cpu.ram[cpu.i + 1] = ~~((cpu.v[X(opcode)] % 100) / 10);
  cpu.ram[cpu.i + 2] = cpu.v[X(opcode)] % 10;
}
...
export function _0xFX55_COPY(cpu: Chip8, opcode: number) {
  const x = X(opcode);
  for (let index = 0; index <= x; index++) {
    cpu.ram[cpu.i + index] = cpu.v[index];
  }
}
